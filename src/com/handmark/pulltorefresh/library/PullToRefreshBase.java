...
_L1:
        ...
        TypedArray typedarray = context.obtainStyledAttributes(attributeset, com.veclink.movnow.healthy_q2.R.styleable.PullToRefresh);
...
_L3:
        int i;
        int j;
        j = Math.round(Math.min(f - f1, 0.0F) / 2.0F);
        i = getHeaderSize();
_L11:
        setHeaderScroll(j);
...
_L4:
        j = Math.round(Math.max(f - f1, 0.0F) / 2.0F);
        i = getFooterSize();
          goto _L11
_L8:
...
_L6:
        float f5 = Math.abs(f);
        if (f5 > (float)mTouchSlop && (!mFilterTouchEvents || f5 > Math.abs(f2)))
        {
            if (mMode.showHeaderLoadingLayout() && f >= 1.0F && isReadyForPullStart())
            {
                mLastMotionY = f3;
                mLastMotionX = f4;
            ...
            if (mMode.showFooterLoadingLayout() && f <= -1F && isReadyForPullEnd())
            {
                mLastMotionY = f3;
                mLastMotionX = f4;
                mIsBeingDragged = true;
...
_L7:
    ...
    {
        Log.d("PullToRefresh", String.format("onSizeChanged. W: %d, H: %d", new Object[] {
...
_L1:
        int j = i1;
_L5:
        Log.d("PullToRefresh", String.format("Setting Padding. L: %d, T: %d, R: %d, B: %d", new Object[] {
            Integer.valueOf(l), Integer.valueOf(j), Integer.valueOf(k), Integer.valueOf(i)
        }));
        setPadding(l, j, k, i);
        return;
_L2:
